  ...
  try {
    ...
    app.delete("/adminPaymentInfo/:id", async (req, res) => {
      try {
        ...
        res.status(500).json({
          success: false,
          message: "An error occurred while deleting payment information",
        });
      }
    });
    // Approve order
    app.patch("/adminPaymentInfo/:id", async (req, res) => {
      try {
        const id = req.params.id;
        console.log("ID:", id);

        // Find the payment info by ID and update the approvedByAdmin field
        const query = { _id: new ObjectId(id) };
        const update = { $set: { approvedByAdmin: true } }; // Using $set to update approvedByAdmin field

        const result = await bkashCollection.findOneAndUpdate(query, update, {
        ...
        res.status(500).json({
          success: false,
          message: "An error occurred while confirming payment information",
